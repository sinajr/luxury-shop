    ...
    <div className="container mx-auto py-8">
      <header className="mb-10 text-center">
        <h1 className="text-4xl font-bold text-primary flex items-center justify-center">
          <Heart className="mr-3 h-10 w-10 text-accent" /> Your Wishlist ({getWishlistItemCount()})
        </h1>
        <p className="mt-2 text-lg text-muted-foreground">Your saved luxury items.</p>
      </header>
        ...
        {isLoadingProducts && wishlistItemIds.length > 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {wishlistProducts.map(item => {
              if (!item) return null; // Guard against undefined items if filtering fails unexpectedly

              let rawPrimaryUrl: string | undefined | null = null;
              if (item.imageUrls && item.imageUrls.length > 0) {
                rawPrimaryUrl = item.imageUrls[0];
              }
              const sanitizedPrimaryUrl = sanitizeImageUrlString(rawPrimaryUrl);
              const primaryImageUrlToDisplay = sanitizedPrimaryUrl || 'https://placehold.co/600x400.png';
              ...
              return (
                <div key={item.id} className="flex flex-col items-center p-4 shadow-md hover:shadow-lg transition-shadow rounded-lg bg-card">
                  ...
                  <div className="flex-grow text-center w-full">
                    ...
                    <p className="text-md font-bold text-accent mt-1">${item.price ? item.price.toFixed(2) : 'N/A'}</p>
                  </div>
