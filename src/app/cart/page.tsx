  ...
  if (!isCartLoaded) {
    ...
    return (
      <div className="flex justify-center items-center h-64">
        ...
        <p className="ml-4 text-xl text-muted-foreground">Loading your cart...</p>
      </div>
    );
  }
    ...
    <div className="container mx-auto py-8">
      <header className="mb-10 text-center">
        <h1 className="text-4xl font-bold text-primary flex items-center justify-center">
          <ShoppingCart className="mr-3 h-10 w-10 text-accent" /> Your Shopping Cart
        </h1>
        <p className="mt-2 text-lg text-muted-foreground">Review your selected luxury items below.</p>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {items.map(item => (
              <Card key={item.id} className="flex flex-col sm:flex-row items-center p-4 shadow-md hover:shadow-lg transition-shadow mb-4">
                ...
                <div className="flex-grow text-center sm:text-left">
                  ...
                  <p className="text-md font-bold text-accent">${item.price.toFixed(2)}</p>
                </div>
          ...
          {items.length > 0 && (
             <div className="flex justify-end mt-4">
                <Button variant="outline" onClick={clearCart} className="text-destructive border-destructive hover:bg-destructive/10 hover:text-destructive">
                    <Trash2 className="mr-2 h-4 w-4" /> Clear Cart ({getCartItemCount()})
                </Button>
            </div>
          )}
        </div>
        ...
        <Card className="lg:col-span-1 shadow-xl h-fit"> 
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex justify-between">
              <p className="text-muted-foreground">Estimated Tax ({(taxRate * 100).toFixed(0)}%)</p>
